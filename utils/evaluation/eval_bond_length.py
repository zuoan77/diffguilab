...
def plot_distance_hist(pair_length_profile, metrics=None, save_path=None):
    ...
    plt.figure(figsize=(6 * len(gt_profile), 6))
    ...
    for idx, (k, gt_distribution) in enumerate(eval_bond_length_config.PAIR_EMPIRICAL_DISTRIBUTIONS.items()):
        ...
        plt.legend(['True', 'Learned'], fontsize=12)
        if metrics is not None:
            plt.title(f'JS. {k}: {metrics["JSD_" + k]:.4f}', fontsize=18)
        else:
            plt.title(k, fontsize=18)
        if idx == 0:
            plt.xlabel('Distance of carbon carbon bond ($\AA$)', fontsize=16)
        elif idx == 1:
            plt.xlabel('Distance of all atom pairs ($\AA$)', fontsize=16)
